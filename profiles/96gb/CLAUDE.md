...
- Keep one primary Claude Code task. Use at most three subagents for genuinely independent workstreams; total active workers must not exceed four.
- Allow at most four concurrent lightweight operations. Limit concurrent writes to unrelated files and never run more than two heavy operations.
- Keep at most 4 task-created browser tabs open and reuse existing tabs.
- Run at most two memory-heavy processes, and do not overlap two full builds or two full test suites. Heavy work includes builds, full test suites, dev servers, browser automation, containers, emulators, indexing, and local models.
- Keep at most three background services in total and stop each as soon as it is no longer needed.
