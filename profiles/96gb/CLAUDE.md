# Performance Profile — 96 GB MacBook
...
- Keep one primary Claude Code task and at most 3 bounded independent subagents; never exceed 4 total active workers.
...
- Keep at most 4 task-created browser tabs; reuse existing tabs.
...
- Limit internal parallelism inside any one build, test, data, or browser command to 3 workers.
- Keep at most 3 task-created background services, and stop each immediately after use.
...
When the project supports the option, apply the 3-worker ceiling explicitly instead of relying on machine defaults:

- Rust: `CARGO_BUILD_JOBS=3` and `cargo ... -j 3`.
- pnpm workspaces: `--workspace-concurrency=3`; install dependencies only once.
- Vitest: `--maxWorkers=3 --minWorkers=1`; Jest: `--maxWorkers=3`.
- Playwright: `--workers=3` and one managed web server; never start a duplicate server manually.
- Python: no pytest-xdist; application multiprocessing/worker pools default to 3. Use a larger pool only briefly when correctness explicitly requires comparing worker counts, then shut it down.
- Swift/Xcode: `swift ... --jobs 3` and `xcodebuild -jobs 3`.
- Make/Go/Gradle: `-j3`, `go ... -p 3`, or `--max-workers=3`.
