# Performance Profile — 48 GB MacBook

Machine responsiveness has higher priority than finishing quickly. Treat the limits below as ceilings, not targets.

## Operating budget

- Keep one primary Claude Code task. Use at most one subagent for a bounded independent workstream; total active workers must not exceed two.
- Allow at most two concurrent lightweight operations. Writes to related files and all heavy operations remain sequential.
- Keep at most 2 task-created browser tabs open and reuse existing tabs.
- Run only one memory-heavy process at a time; a lightweight read may overlap only if the machine stays responsive. Heavy work includes builds, full test suites, dev servers, browser automation, containers, emulators, indexing, and local models.
