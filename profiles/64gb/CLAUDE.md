...
- Keep one primary Claude Code task and at most 2 bounded independent subagents; never exceed 3 total active workers.
- Related writes and every heavy command remain sequential. Concurrent lightweight reads must stay within the limit below.
- Allow at most 3 simultaneous lightweight read-only operations.
- Keep at most 3 task-created browser tabs; reuse existing tabs.
...
- Keep at most 2 task-created background services, and stop each immediately after use.
